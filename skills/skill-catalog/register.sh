...
done < <(find "$SKILLS_DIR" -name "SKILL.md" -type f -not -path "*/skill-catalog/*" -print0 | sort -z)
...
# 生成 INDEX.md
{
    echo "# Skills Index"
    echo ""
    echo "> 自动生成，勿手动编辑。运行 \`bash skills/skill-catalog/register.sh\` 更新。"
    ...
    echo "3. 新增/删除 skill 后，运行 \`bash skills/skill-catalog/register.sh\` 更新"
